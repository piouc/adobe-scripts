...
!function() {
/*!*********************************!*\
  !*** ./src/rename-artboards.ts ***!
  \*********************************/
/// <reference types="types-for-adobe/Illustrator/2015.3"/>
(function () {
  var win = new Window('dialog', 'rename', undefined);
  var artboards = Array.from(app.activeDocument.artboards);
  var names = artboards.map(function (artbord) {
    return artbord.name;
  });
  var textarea = win.add("edittext", [30, 30, 400, 1000], names.join('\n'), {
    multiline: true
  });
  var button = win.add("button", [50, 30, 150, 60], "Rename");

  button.onClick = function () {
    var names = textarea.text.split('\n');

    if (names.length < artboards.length) {
      alert('less');
    } else {
      artboards.map(function (artbord, i) {
